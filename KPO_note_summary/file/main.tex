...
\usepackage[group-minimum-digits=0,table-figures-decimal=0,table-number-alignment=center]{siunitx}
\usepackage[indent=10pt, skip=2pt plus1pt]{parskip} % Change paragraph spacing
\usepackage{makeidx}
...
\author{Diego Ruiz}
\email{diego.ruiz@alice-bob.com}%
\affiliation{Alice \& Bob, 53 Bd du Général Martial Valin, 75015 Paris, France}
\affiliation{Laboratoire de Physique de l'\'Ecole Normale Sup\'erieure, Inria, ENS, Mines ParisTech, Universit\'e PSL, Sorbonne Universit\'e, Paris, France}
...
The simulations were performed in the Kerr eigenstate basis with a truncation of 20 eigenstates. To evaluate the bit-flip probability, we use the observable $\hat Z  = \text{sign}(\adag + \hata)$. The bit-flip rate is then evaluated by fitting $\frac{1}{2}(1~-~e^{-2 \Gamma t})$.
